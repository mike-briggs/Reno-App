...
class Browse extends Component {
  ...
  render() {
    ...
    const tabs = ['Contractors', 'Inspirations', 'FAQ'];
        ...
        >
          <Block flex={false} row space="between" style={styles.categories}>
            {categories.map(category => (
              <TouchableOpacity
                onPress={() => navigation.navigate('Inspirations')}
              >
                <Card center middle shadow style={styles.category}>
                  <Badge margin={[0, 0, 15]} size={50} color="rgba(29, 38, 45,0.07)">
                    <Image source={category.image} />
                  </Badge>
                  <Text medium height={20}>{category.name}</Text>
                  <Text gray caption>{category.count} contractors</Text>
                </Card>
              </TouchableOpacity>
            ))}
          </Block>
